...
_TABLE_SETTINGS_TEXT = {"vertical_strategy": "text", "horizontal_strategy": "text", "min_words_vertical": 2, "min_words_horizontal": 2}
...
class TableDetector:
    ...
    def detect(self, pdf_path: str, page_numbers: list[int]) -> list[DetectedTable]:
        ...
        def process_table(dt: DetectedTable | None):
            ...
            if not anchor_found and (has_identity and has_quantity and (has_unit or has_code)):
                anchor_found = True  
    ...
    def _build_table(self, page_number, raw, source) -> DetectedTable | None:
        ...
        for hidx in range(min(10, len(grid))):
            ...
            if "1" in digits and "2" in digits and "7" in digits:
                ...
                for c_idx, val in enumerate(row):
                    v = val.strip()
                    if v == "1": cmap["item_no"] = c_idx
                    elif v == "2": cmap["name"] = c_idx
                    elif v == "4": cmap["code"] = c_idx
                    elif v == "6": cmap["unit"] = c_idx
                    elif v == "7": cmap["quantity"] = c_idx
        ...
        dt = DetectedTable(page_number=page_number, header=header, rows=data_rows, column_map=best_map, source=source)
    ...
    def _infer_structure_from_data(self, grid: list[list[str]]) -> dict[str, int] | None:
        ...
        def frac_unit(vals):
            if not vals: return 0.0
            return sum(1 for v in vals if v.lower().rstrip(".") in units or v.lower() in units) / len(vals)
        ...
        def frac_code(vals):
            ...
            for v in vals:
                ...
                if ("-" in v or "/" in v) and digits >= 3: n += 1
            return n / len(vals)

        def frac_cyr_text(vals):
            if not vals: return 0.0
            n = sum(1 for v in vals if len(v) >= 8 and any('\u0400' <= ch <= '\u04FF' for ch in v))
            return n / len(vals)
        ...
        for ci in range(width):
            vals = col_values(ci)
            if len(vals) < max(2, len(data_rows) * 0.2): continue
            populated.append(ci)
            avg_len = sum(len(v) for v in vals) / len(vals)

            ft = frac_cyr_text(vals)
            if ft > 0.3 and avg_len > name_score: name_score, name_c = avg_len, ci

            fc = frac_code(vals)
            if fc > 0.3 and fc > code_score: code_score, code_c = fc, ci

            fu = frac_unit(vals)
            if fu > 0.3 and fu > unit_score: unit_score, unit_c = fu, ci
        ...
        for ci in populated:
            if ci in (name_c, code_c, unit_c): continue
            if is_position_column(col_values(ci)): itemno_c = ci; break

        for ci in populated:
            if ci in (name_c, code_c, unit_c, itemno_c): continue
            vals = col_values(ci)
            fi = frac_int(vals)
            if fi > 0.6:
                score = fi + (0.5 if unit_c is not None and ci > unit_c else 0)
                if score > qty_score: qty_score, qty_c = score, ci

        inferred = {}
        if name_c is not None: inferred["name"] = name_c
        if code_c is not None and code_c != name_c: inferred["code"] = code_c
        if unit_c is not None and unit_c not in (name_c, code_c): inferred["unit"] = unit_c
        if qty_c is not None and qty_c not in (name_c, code_c, unit_c): inferred["quantity"] = qty_c
        if itemno_c is not None and itemno_c not in (name_c, code_c, unit_c, qty_c): inferred["item_no"] = itemno_c

        has_identity = "name" in inferred or "description" in inferred
        if has_identity and len(inferred) >= 2: return inferred
        return None

    def _has_real_quantity_or_price(self, col_map, data_rows) -> bool:
        if "price" in col_map: return True
        units = {"шт", "шт.", "штук", "компл", "комплект", "компл.", "к-т", "м", "м.", "м2", "м²", "м3", "м³", "пог.м", "п.м", "м.п", "кг", "г", "т", "л", "уп", "упак", "пара", "набор", "рулон", "pcs", "pc", "set", "kit", "unit", "ea"}
        ...
        if qcol is not None:
            ...
            if vals:
                def is_clean_count(v: str) -> bool:
                    if "/" in v or ":" in v or "-" in v: return False
                    try: float(v.replace(",", ".").replace("*", "").strip()); return True
                    except ValueError: return False
                if sum(1 for v in vals if is_clean_count(v)) / len(vals) >= 0.6: return True

        ucol = col_map.get("unit")
        if ucol is not None and "code" in col_map:
            uvals = col_nonempty(ucol)
            if uvals:
                if sum(1 for v in uvals if v.lower().rstrip(".") in units or v.lower() in units) / len(uvals) >= 0.5: return True
        return False

    def _identity_column_is_valid(self, id_col, data_rows) -> bool:
        if id_col is None or not data_rows: return False
        values = [(r[id_col] or "").strip() for r in data_rows if id_col < len(r) and (r[id_col] or "").strip()]
        if not values: return False
        is_large_table = len(data_rows) >= 40
        return (len(values) / len(data_rows) >= (0.30 if is_large_table else 0.40)) and (sum(len(v) for v in values) / len(values) >= (3.0 if is_large_table else 5.0))

    def _table_confidence(self, dt: DetectedTable) -> float:
        score = 0.0
        fields = set(dt.column_map)
        if "name" in fields or "description" in fields: score += 0.4
        if "quantity" in fields: score += 0.2
        if "unit" in fields: score += 0.1
        if "code" in fields: score += 0.1
        if "price" in fields: score += 0.1
        if len(dt.header) >= 3: score += 0.05
        if len(dt.rows) >= 2: score += 0.05
        if dt.source == "ocr": score *= 0.8
